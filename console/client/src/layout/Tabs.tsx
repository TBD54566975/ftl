...
export const Tabs = () => {
  ...
  return (
    <div className={`flex-1 flex flex-col rounded`}>
      <TabComponent.Group selectedIndex={tabs.findIndex((tab) => tab.id === activeTabId)} onChange={handleChangeTab}>
        <div>
          <TabComponent.List className={`flex items-center rounded-t ${headerTextColor}`}>
            {tabs.map((tab) => {
              return (
                <TabComponent key={tab.id} className='flex items-center mr-1 relative cursor-pointer' as='span'>
                  <span
                    className={`px-4 py-2 rounded-t ${tab.isClosable ? 'pr-8' : ''} ${
                      activeTabId === tab.id ? `${selectedTabStyle}` : `${unselectedTabStyle}`
                    }`}
                  >
                    {tab.label}
                  </span>
        ...
        <div className={`flex-1 overflow-y-scroll ${panelColor}`}>
          <TabComponent.Panels>
            {tabs.map((tab) => (
              <TabComponent.Panel key={tab.id}>{tab.component}</TabComponent.Panel>
            ))}
          </TabComponent.Panels>
        </div>
      </TabComponent.Group>
