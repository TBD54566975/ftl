...
pub fn build() {
    ...
    let token_stream = quote::quote! {
        pub fn call_immediate(ctx: ::ftl::Context, module: String, verb: String) -> ::std::pin::Pin<Box<dyn ::std::future::Future<Output = ()> + Send + Sync>> {
             let fut = async move {
    ...
    let out_path = Path::new(&out_dir).join("lookup.rs");
