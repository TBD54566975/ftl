...
CREATE TYPE controlplane_state AS ENUM (
    'live',
    'dead'
    );
...
(
    ...
    created   TIMESTAMPTZ        NOT NULL DEFAULT (NOW() AT TIME ZONE 'utc'),
    last_seen TIMESTAMPTZ        NOT NULL DEFAULT (NOW() AT TIME ZONE 'utc'),
    state     controlplane_state NOT NULL DEFAULT 'live',
    endpoint  VARCHAR            NOT NULL
);

CREATE UNIQUE INDEX controlplane_endpoint_not_dead_idx ON controlplane (endpoint)
    WHERE state <> 'dead';
...
(
    ...
    request_id    BIGINT      NOT NULL REFERENCES ingress (id) ON DELETE CASCADE,
