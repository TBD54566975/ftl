...
impl Plugin {
    pub fn generate_from_input(input: &[u8]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
        let req = pluginpb::GenerateRequest::decode(input)?;
        let resp = Self::handle_generate(req)?;
        Ok(resp.encode_to_vec())
    }

    fn handle_generate(req: pluginpb::GenerateRequest) -> Result<pluginpb::GenerateResponse, Box<dyn std::error::Error>> {
...
fn generate_schema(request: &pluginpb::GenerateRequest) -> Result<schemapb::Module, Box<dyn std::error::Error>> {
...
fn to_verb(query: &pluginpb::Query, module_name: &str) -> schemapb::Decl {
    let request_type = if !query.params.is_empty() {
        Some(to_schema_ref(module_name, &format!("{}Request", query.name)))
    } else {
        None
    };
    ...
    let response_type = if query.cmd == ":exec" {
        ...
        Some(to_schema_ref(module_name, &format!("{}Response", query.name)))
    };
...
fn to_verb_request(query: &pluginpb::Query) -> schemapb::Decl {
    schemapb::Decl {
        value: Some(schemapb::decl::Value::Data(schemapb::Data {
            name: format!("{}Request", query.name),
            export: false,
...
fn to_verb_response(query: &pluginpb::Query) -> schemapb::Decl {
    schemapb::Decl {
        value: Some(schemapb::decl::Value::Data(schemapb::Data {
            name: format!("{}Response", query.name),
            export: false,
            type_parameters: Vec::new(),
...
fn get_module_name(req: &pluginpb::GenerateRequest) -> Result<String, Box<dyn std::error::Error>> {
    let codegen = req.settings
        .as_ref()
        .ok_or("Missing settings")?
        .codegen
        .as_ref()
        .ok_or("Missing codegen settings")?;

    let options_str = String::from_utf8(codegen.options.clone())
        .map_err(|e| format!("Invalid UTF-8 in options: {}", e))?;
    
    let options: serde_json::Value = serde_json::from_str(&options_str)
        .map_err(|e| format!("Failed to parse JSON options: {}", e))?;

    options.get("module")
        .and_then(|v| v.as_str())
        .ok_or("Missing module name in options")
        .map(|s| s.to_string())
        .map_err(|e| e.into())
}
