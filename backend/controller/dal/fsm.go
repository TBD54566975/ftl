...
func (d *DAL) StartFSMTransition(ctx context.Context, fsm schema.RefKey, executionKey string, destinationState schema.RefKey, request json.RawMessage, retryCount int, minBackoff time.Duration, maxBackoff time.Duration) (err error) {
	...
	asyncCallID, err := d.db.CreateAsyncCall(ctx, sql.CreateAsyncCallParams{
		...
		Request:           request,
		RemainingAttempts: int32(retryCount),
		Backoff:           minBackoff,
