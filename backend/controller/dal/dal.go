...
func (d *DAL) CreateDeployment(ctx context.Context, language string, moduleSchema *schema.Module, artefacts []DeploymentArtefact, ingressRoutes []IngressRoutingEntry, cronJobs []model.CronJob) (key model.DeploymentKey, err error) {
	...
	qtx := sql.New(d.db.Conn()).WithTx(tx.Tx())
	...
	cronjobsqtx := cronjobssql.New(d.db.Conn()).WithTx(tx.Tx())
...
func (*DAL) checkForExistingDeployments(ctx context.Context, tx *sql.Queries, moduleSchema *schema.Module, artefacts []DeploymentArtefact) (model.DeploymentKey, error) {
