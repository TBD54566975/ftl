...
CREATE TABLE topics (
    ...
    module BIGINT NOT NULL REFERENCES modules(id),
...
CREATE UNIQUE INDEX topics_module_name_idx ON topics(module, name);
...
CREATE TABLE topic_subscribers (
   ...
   -- Fully-qualified reference to the consumer sink.
   sink_ref VARCHAR NOT NULL
);

-- Mapping functions for a topic.
--
--
CREATE TABLE topic_mappers (
   id BIGINT NOT NULL GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
   created_at TIMESTAMPTZ NOT NULL DEFAULT (NOW() AT TIME ZONE 'utc'),

   input_topic_subscriptions_id BIGINT NOT NULL REFERENCES topic_subscriptions(id),
   output_topic_id BIGINT NOT NULL REFERENCES topics(id),

   verb_ref VARCHAR NOT NULL
);
