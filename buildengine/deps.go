...
func extractDependencies(module Module) ([]string, error) {
	...
	case "swift":
		return extractSwiftFTLImports(module.Config.Module, module.Config.Dir)
...
func extractSwiftFTLImports(moduleName, dir string) ([]string, error) {
	// dependencies := map[string]bool{}
	// fset := token.NewFileSet()
	// err := WalkDir(dir, func(path string, d fs.DirEntry) error {
	// 	if !d.IsDir() {
	// 		return nil
	// 	}
	// 	// TODO: Skip appropriate directories
	// 	// if strings.HasPrefix(d.Name(), "_") || d.Name() == "testdata" {
	// 	// 	return ErrSkip
	// 	// }
	// 	pkgs, err := parser.ParseDir(fset, path, nil, parser.ImportsOnly)
