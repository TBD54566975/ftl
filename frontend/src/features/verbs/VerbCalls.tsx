        ...
        <table className='mt-6 w-full text-left'>
          ...
          <tbody className='divide-y divide-black/5 dark:divide-white/5'>
            {calls.map((call, index) => (
              <tr key={index} onClick={() => handleClick(call)} className='cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800'>
                ...
                <td className='hidden py-4 pl-0 pr-4 sm:table-cell sm:pr-8'>
                  <div className='flex gap-x-3'>
                    <div className='font-mono text-sm leading-6 text-gray-500 dark:text-gray-400'>{formatTimestamp(call.timeStamp)}</div>
                  </div>
                </td>
