...
class ModuleGenerator() {
  fun run(schemas: List<PullSchemaResponse>, outputDirectory: File, moduleName: String) {
    println("generating files for module: ${moduleName}")
    schemas.filter { it.module_name != moduleName }.forEach { generateModule(it, outputDirectory, moduleName) }
  }

  private fun generateModule(schema: PullSchemaResponse, outputDirectory: File, moduleName: String) {
    val file = FileSpec.builder("ftl.${moduleName}", schema.module_name.capitalized())
