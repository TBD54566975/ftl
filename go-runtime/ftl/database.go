...
func (DefaultPostgresDatabaseConfig) pgdb() {} //nolint:unused
...
// Name returns the name of the database.
func (d DatabaseHandle[DatabaseConfig]) Name() string { return d.name }

// Type returns the type of the database, e.g. "postgres"
func (d DatabaseHandle[DatabaseConfig]) Type() DatabaseType {
	return d._type
}

// String returns a string representation of the database handle.
func (d DatabaseHandle[DatabaseConfig]) String() string {
	return fmt.Sprintf("database %q", d.name)
}

// Get returns the SQL DB connection for the database.
func (d DatabaseHandle[DatabaseConfig]) Get(ctx context.Context) *sql.DB {
	db, err := d.db.Get(ctx)
	if err != nil {
		panic(err)
	}
