...
func Extract(pass *analysis.Pass, node *ast.FuncDecl, obj types.Object) optional.Option[*schema.Verb] {
	...
	if !common.ApplyMetadata[*schema.Verb](pass, obj, func(md *common.ExtractedMetadata) {
		...
		for idx, param := range node.Type.Params.List {
			paramObj := common.GetObjectForNode(pass.TypesInfo, param.Type)
			r, err := resolveResource(pass, paramObj, param.Type, getParamResourceType(pass, paramObj))
			if err != nil {
			...
			switch r.typ {
			case verbClient:
				common.MarkIncludeNativeName(pass, paramObj.MustGet(), r.ref)
				verb.AddCall(r.ref)
			case databaseHandle:
...
func resolveResource(pass *analysis.Pass, obj optional.Option[types.Object], typ ast.Expr, rType resourceType) (*resource, error) {
	var ref *schema.Ref
	switch rType {
	case none:
		return nil, nil
	...
	case mappedHandle:
		...
		resolved, err := resolveResource(pass, handleObj, handle, handleType)
		if err != nil {
			return nil, err
		}
